#!/usr/bin/env bash
# Usage: append-and-publish.sh <source-image-path> <title> <universe>
...
ENTRY=$(node scripts/png-to-pixel.js "$SRC" "$TITLE" "$UNIVERSE" "$SIZE" "$PALETTE")
...
const entry = $ENTRY;
// Guard against dupes (same title + heroes category)
if (arr.some(p => p.category === 'heroes' && p.title === entry.title)) {
